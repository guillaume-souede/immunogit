...
def download_model_with_metadata(model_data, base_directory):
    ...
    try:
        ...
        if "BIOM" in model_id:
            directory = os.path.join(base_directory, "Curated_models")
        else:
            directory = os.path.join(base_directory, "No_Curated_models")
...
def main():
    # Requête mise à jour
    query = (
        'immun* AND modellingapproach:"logical model"'
    )

    # Répertoire principal
    base_directory = "downloaded_models_SBMLQual"
